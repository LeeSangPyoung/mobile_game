...
(function(){
  ...
  // ---- 상태 직렬화/적용 (전역 castles/armies). 성은 위치(cx,cy)로 매칭 → 순서 무관 ----
  var _ckey = function(x,y){ return Math.round(x*10000)+','+Math.round(y*10000); };
  MP.serialize = function(){
    return {
      c: castles.map(function(c){ return { o:c.owner, p:c.primary, w:Math.round(c.wallHP), t:[c.troops.spear|0,c.troops.cavalry|0,c.troops.archer|0], ct:!!c._contested, cx:c.x, cy:c.y }; }),
      a: armies.map(function(a){ return { o:a.owner, u:a.unit, r:a.troops, x:Math.round(a.x), y:Math.round(a.y), s:a.seed, m:a._marchAngle, d:a.dying?1:0, fa:a.fallAngle, dt:a.deathT, dr:a._deathTroops, sh:a.shooting?1:0, st:a._start }; }),
      ck: castles.length
    };
  };
  MP.apply = function(s){
    ...
    // 게스트 성을 위치키로 인덱싱
    var byKey = {};
    for(var i=0;i<castles.length;i++) byKey[_ckey(castles[i].x, castles[i].y)] = castles[i];
    for(var k=0;k<s.c.length;k++){ var sc=s.c[k];
      var c = byKey[_ckey(sc.cx, sc.cy)];
      if(!c) continue;  // 위치 매칭 안되면 스킵(스테이지 어긋남)
      c.owner=sc.o; c.primary=sc.p; c.wallHP=sc.w; c.troops={spear:sc.t[0],cavalry:sc.t[1],archer:sc.t[2]}; c._contested=sc.ct;
    }
    // 부대는 호스트 목록으로 재구성. target은 위치가 없으니 첫 성으로(렌더 marchAngle fallback용)
    armies.length = 0;
    for(var j=0;j<s.a.length;j++){ var sa=s.a[j];
      armies.push({ owner:sa.o, unit:sa.u, troops:sa.r, x:sa.x, y:sa.y, seed:sa.s, _marchAngle:sa.m,
        ...
        target:castles[0], isArmy:true, atkBonus:1 });
    }
  };
  ...
  // ---- 게임이 blur(포커스 상실) 시 _appHidden=true로 루프를 멈추는 걸 무력화. ----
  //   멀티 중에는 포커스와 무관하게 항상 돌아야 함(2뷰/2창). 화면에 보이면 rAF는 정상.
  ...
  function broadcastQ(){ if(MP.inQueue) try{ BC.postMessage({t:'Q', id:MP.qid, jt:MP.qtime}); }catch(_){}}
  ...
  function enterQueue(){
    ...
    MP.qtime = Date.now();                 // 큐 입장 시각 — 빠른 쪽이 호스트
  ...
  function onPeerQ(peerId, peerTime){
    if(!MP.inQueue || MP.role || peerId===MP.qid) return;
    MP.peer = peerId;
    // 먼저 큐 들어온 쪽(=qtime 작은 쪽)이 호스트. 동시면 qid로 타이브레이크.
    var iAmHost = (MP.qtime < peerTime) || (MP.qtime===peerTime && MP.qid < peerId);
    if(iAmHost) becomeHost(peerId);
    // 아니면 대기 → 호스트의 START_MP 수신 시 게스트로
  }
