...
<div id="wrap">
  ...
  <div id="hud">
    <div style="color:#58a6ff">🔵 아군 성 <b id="c1">-</b> 병력 <b id="t1">-</b></div>
    <div style="color:#f85149">🔴 적 성 <b id="c2">-</b> 병력 <b id="t2">-</b></div>
    <div class="spacer"></div>
...
(function(){
  ...
  // 시점 기준 상대색: 내 진영=파랑(아군), 중립=회색, 그 외=빨강(적)
  const col = (o) => o===0 ? '#8b949e' : (o===mySide ? '#58a6ff' : '#f85149');
  ...
  let guestWallMax=[]; // 게스트: 미러 엔진에서 계산한 성벽최대치(정적)
  // 렌더 소스: 호스트는 host.eng(권위), 게스트는 guest.snap+guest.map
  function castlesView(){
    if (role==='host') return host.eng.castles;
    if (guest && guest.snap && guest.map) return guest.map.castles.map((mc,i)=>({ x:mc.x,y:mc.y,isHome:mc.isHome,size:mc.size,owner:guest.snap.castles[i].owner,troops:guest.snap.castles[i].troops,wallHP:guest.snap.castles[i].wallHP,wallMax:guestWallMax[i]||1 }));
    return [];
  }
  ...
  function startGuest(){
    ...
    guest=new GuestMatch(ch, {
      onStart:(m)=>{ mySide=m.youSide; map=m.map; const e=new SimEngine(map,0); mapPx={w:e._pxW,h:e._pxH}; guestWallMax=e.castles.map(c=>c.wallMax);
  ...
  function loop(){
    ...
    const now=performance.now(); let dt=Math.min((now-lastT)/1000,0.5); lastT=now; // 백그라운드 스로틀 시 최대 0.5s 따라잡기
  ...
  function render(){
    ...
    for(const a of armiesView()){
      ...
      ctx.fillStyle=col(a.owner); ctx.globalAlpha=a.dying?0.35:1; ctx.fill();
    ...
    cs.forEach((c,i)=>{
      ...
      ctx.beginPath(); ctx.arc(x,y,r,0,Math.PI*2); ctx.fillStyle=col(c.owner); ctx.fill();
    ...
    // HUD (시점 기준: 아군=내 진영, 적=상대)
    const enemy = mySide===1?2:1;
    const mc=cs.filter(c=>c.owner===mySide), ec=cs.filter(c=>c.owner===enemy);
    const av=armiesView();
    ...
    $('c1').textContent=mc.length; $('t1').textContent=sum(mc,mySide);
    $('c2').textContent=ec.length; $('t2').textContent=sum(ec,enemy);
